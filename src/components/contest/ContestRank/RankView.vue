                ...
                <div class="ranking" style="width:100%;">
                    <table class="ui small celled table" id='rank'>
                        ...
                        <transition-group name="list-complete" tag="tbody">
                            <tr :key="row.user_id" class="list-complete-item" style="cursor: grab!important;"
                                ...
                                <td style="text-align: center" v-if="hasPrivilege">{{row.totalSubmitTime === 0 ? "未提交" : `${Math.trunc(row.ac * 10000 / row.totalSubmitTime) / 100}%`}}</td>
                    ...
                    <table :style="'display:none;vnd.ms-excel.numberformat:@'" id="save">
                        ...
                        <tr :key="key" v-for="(row,key) in submitter">
                            ...
                            <td align="center" v-if="hasPrivilege">{{row.totalSubmitTime === 0 ? "未提交" : `${Math.trunc(row.ac * 10000 / row.totalSubmitTime) / 100}%`}}</td>
...
export default class RankView extends Mixins(mixins) {
        @Prop({ default: "0" }) cid!: string
        ...
        rankClass (rank: any, row: Submitter) {
            ...
            else if (row.ac >= Math.ceil(this.total * 0.6)) {
                return "ui orange";
            }
            else {
                return "ui white";
        ...
        exportXLS () {
            ...
            XLSContentHTML += "<tr>Contest " + this.cid + " " + this.title + "</tr>";
        ...
        async initData (cid: string) {
            ...
            let dataSet: any = [];
            const users = new Set();
            convertFlag = cidArr.length > 1;

            const work = (cid: string) => {
                return this.axios.get(`/api/scoreboard/${cid}`)
                    .then(({ data }) => {
                        return data;
                    });
            };
            ...
            if (cidArr.length > 1) {
                ...
                try {
                    const response = await Promise.all(cidArr.map((e: string) => work(e)));
